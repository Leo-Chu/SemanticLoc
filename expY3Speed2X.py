...
from network_models import TL_net, TL_net2, TL_net3, TL_net4
import os
...
def load_data(arg, logger):
    ...
    if arg.input_norm == 'default':
        ...
        print('size of train_data0 is', train_data0.shape)
        temp1, temp2 = [], []
    ...
    for epoch in range(arg.epochs):
        ...
        for i in range(len_dataloader):
            ...
            if arg.method == 'DCNN':
                ...
                reg_error = loss_fn(loc_s_LOC.float(), logit_output.float())  # + 1e-3*para_l2_norm   pure regression
                ...
                logit_output, _, dis_err = nets(csi_s, csi_t, arg)
                reg_error = loss_fn(loc_s_LOC.float(), logit_output.float())
...
if __name__ == '__main__':
    ...
    args.pretrained = True  # False True
...
### network training and test
    ...
    args.data_root = file0 + '/results/' + 'EXP1' + '/DataDifSpeed/2X/'

    args.input_norm = 'antenna_wise'

    args.name = 'EXP1difSpeed' + '_' + args.input_norm + '_' + args.method
    logger = get_logger(args.name)
    train_dataset, test_dataset = load_data(args, logger)

    args.model_root = file0 + '/results/' + 'EXP1' + '/DataDifSpeed/2X/' + '/saved_models/' + args.method + '/' + str(args.gammar) + '/'

    args.estimations_root = file0 + '/results/' + 'EXP1' + '/DataDifSpeed/2X/' + '/estimations/' + args.method + '/' + str(args.gammar) + '/'
